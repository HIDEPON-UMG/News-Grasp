...
readinessの実行順序は、`final admission`発行、`issue-a`によるpolicyだけの発行、official wrapper内の`validate-issued`によるparent authority再検証とtransition receipt／SQLite ledgerの一回限り生成、runner起動の順に固定する。`issue-a`はreceiptやledgerを作成しない。生成pack、添付prompt、過去handoff、既存artifactはユーザーの同意証拠ではなく、明示的なactual-user confirmationなしに実行authorityまたはstart authorityへ昇格してはならない。

release reflectionはpush、remote `refs/heads/main`一致、install、runtime parityが確認された後に発行する。push前にpost-push Greenを要求せず、post-push verificationはpush後の別証跡として扱う。full pytest、TDD、broker availabilityは、ユーザーが明示的に合意していないpackから開始条件へ追加しない。現行の内部final-admission契約は15 Requirement、4 domain scopes、60 fixtures、150 pair cases、240 traceability cells、exact 211 nodesで維持する。
