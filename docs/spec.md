...
| Article data / schema / tags | record schema、frontmatter、Obsidian tags、entities / topics / industries / events の意味論を維持する。 | `data/articles.jsonl`、digest frontmatter、tag / session URL / article append 成果物。 | `tools.validate_record`、`tests/test_validate_record.py`、tag / session URL / article append 系契約テスト。 |
| Digest / category schedule | `tools.publish_inventory.scheduled_category_ids(issue)` を唯一の必須カテゴリ正本とし、対象カテゴリ、休載条件、記事数不足時の refill / quarantine、`data/search_audit` 契約を更新する。 | `digest/daily/`、日付 docs、カテゴリ docs、search audit。 | `tools.validate_daily_quality --date <date> --docs-root docs --require-deepdive`、`tools.publish_inventory --date <date> --kind categories --json`、カテゴリ presence / search audit 契約テスト。 |
...
| DeepDive | md、HTML、関係図、日付ページからの導線、公開 inventory、TTS audio refs を更新する。 | `digest/DeepDive/`、`docs/deepdive/`、DeepDive audio、日付 docs link。 | `tools.validate_deepdive_urls digest/DeepDive/<date>-DeepDive.md`、`tools.validate_daily_quality --date <date> --docs-root docs --require-deepdive`、DeepDive presence / relation layout テスト、公開 URL sentinel。 |
