...
06:00 Codex automation の通常日次公開は `$news-grasp-direct-mainline` と固定Python 3.12の `tools.news_grasp_daily_launcher` だけを入口にする direct 本線である。launcher内部の状態更新は `tools.news_grasp_direct_runtime` が所有する。旧 runner、NoPublish、fallback publish、runner state、readiness、durable goal、audit/report、URL 200単独、publish-status単独、commit/push単独は、この route の public completion authority ではない。legacy runner/recovery 章と矛盾する場合、06:00 direct 本線については本節を優先する。
...
`NG-DAILY-45M-20260902` は、記事品質を削らずに重複判定とRelease-only検証を日次critical pathから除外する。ScheduledProductionが起動できるentryは固定Python 3.12の `tools.news_grasp_daily_launcher` 一つだけであり、同一process memoryのwriter leaseで `static_check` → `scoped_contract_unit` → `current_issue_integration` → `external_publication` → `consumer_public_verification` → `atomic_completion` を順に一回ずつ実行する。個別operation CLI、raw/full pytest、historical corpus、Playwright全件、crash/replay/drift、final NoPublish E2E、Release gate、raw Python、未登録routeはspawn前にfail-closedにし、writer/fencing capabilityをstdout・環境変数・receiptへ投影しない。`protectedRelease`の通常Daily再実行はstate作成前に拒否し、production runtime/Release stateはWindows Known Folderへ固定する。Release ledgerはMAC付きhash-chainとprocess横断lockを用いる。committed sourceではGreen event・promotion receipt・発行eventを同一排他区間でexactly onceに束縛し、staged candidateではGreen eventを先に封印したうえで、exact tree/direct parent commit確認後の別transactionが同じrelease eventへpromotionをexactly onceで付加する。

installer、Task Scheduler dispatch、deadman recovery、Release-only NoPublishは、廃止済み `news-grasp-runner.ps1` を実行依存に持たない。05:55 bootstrapは証拠repoから対話なし・有界時間で `origin/main` を取得し、remote commit SHAを固定して既存transactional recovery内でproduction runtimeを収束させ、active generationを再検証する。scheduled dispatchは固定Pythonから、cwdをproduction runtimeへ固定し継承Python module pathを除去したうえで `tools/news_grasp_daily_launcher.py` を一回だけ起動する。deadmanはinstalled stable task launcherのexact `dispatch --schedule-id news-grasp-daily-v1 --intent reconcile` actionへ収束する。Release-only NoPublishは実P08 isolation consumerでcleanroom差分を検証し、`tools/news_grasp_release_nopublish.py` を隔離worktree・隔離stateで起動して、外部adapter呼出し0と保護済み2026-09-02の再生成・再配信0を機械的に保証する。
...
Global HighCost capabilityとのproduct境界は `NEWS_GRASP_HIGH_COST_BINDING_V1` とする。正規installerはGlobalの `HIGH_COST_CAPABILITY_DESCRIPTOR_V1` と共通 `high_cost_capability_adapter.py probe/resolve` がGreenであることをlive mutation前に確認し、descriptor path/hash/generation、adapter path/hash、workspace rootをproduct-local receiptへ束縛してlive binへatomic配置する。authority、budget、goal、audit terminalは複製しない。Scheduled Task Actionはbinding pathとreceipt SHAを固定し、task launcher、bootstrap、watcher、runner、model wrapper、audit/recovery controllerは同じ2値を明示伝播して各境界で再検証する。productionの `ScheduledProduction` / `ScheduledRecoveryFull` は旧 `HighCostWorkspaceRoot` / `HighCostBudgetToolPath` と全ambient workspace rootを拒否する。隔離testも正規schemaの一時bindingを生成して同じconsumerへ注入し、test専用root fallbackをproduction moduleへ持ち込まない。descriptor、adapter、binding fileのいずれかが変化した場合は生成・model・publish前に `HIGH_COST_IDENTITY_DRIFT` で停止し、workspace binding欠落、broker不在、operation admission欠落を別reasonとして保持する。

Scheduled Task / runner / bootstrap / deadman はTask Actionから最深childまでno-consoleで動作する。Taskは`pythonw.exe`とversioned launcherを使い、launcher childは`CREATE_NO_WINDOW`を強制する。正規日次roleはBootstrap=05:55 enabled/IgnoreNew、Production=06:00のみ enabled/IgnoreNew、Deadman=06:40開始+PT1H/P1D enabled/IgnoreNewとし、Pull/legacy RunnerはDisabledを維持する。Deadman launcherは正規recovery 90分とcloseout余裕を壊さない100分でowned child treeを終了し、Task Schedulerの`ExecutionTimeLimit=PT1H45M`を二段目の上限とする。enable/disableはこのrole authorityと明示的な移行transactionだけが変更でき、no-console移行だけを理由に状態を変えない。
...
| Scheduled automation title / task lifecycle | 実行スレッド生成前に 05:59 JST の `News-Grasp Title Materializer` が Asia/Tokyo の対象日を exact `YY/MM/DD News-Grasp 臨時本線日次バッチ 6:00 記事作成・公開` へ materialize する。source template は日付を保持せず、installed TOML と Codex App DB の `name` だけを更新し、prompt、schedule、model、reasoning、cwd、target を変更しない。本文の `set_thread_title`、正規表現、日時 placeholder、automation 内の自己更新は表示名の実装に使わない。 | `automation/news-grasp-6-40/automation.toml.template`、`tools/news_grasp_title_materializer.py`、installed automation TOML、Codex App DB、`News-Grasp Title Materializer` Scheduled Task、materialization receipt、実行スレッド title。 | materializer unit / SQLite preservation / TOML preservation テスト、installed/App DB exact一致、専用 Scheduled Task の hidden・05:59・single-instance・non-runner action、後続 canary の新規 thread title exact一致。 |
| Runner / state / recovery | full run / RecoverOnly / fallback publish / OK marker / distribution state / live ops readiness の遷移を更新する。repair runner は structured issue ledger の direction / evidence / selected artifacts を保持し、registry の `noop` / `not_applicable` を repair 成功として扱わない。matrix が所有する `verify_gate` / `allowed_artifacts` を registry metadata で上書きせず、registry 側の広い実装 scope は診断 metadata として分離する。`blocked_articles_only_record_incomplete` と `blocked_digest_only_ambiguous` を generic failure に丸めず、同じ gate の再検証 Green まで次 stage へ進まない。通常 publish_complete は repo/live runner SHA 一致、repo/live watcher SHA 一致、repo/live bootstrap SHA 一致、root TaskPath上のProduction 06:00のみ/IgnoreNew、Bootstrap 05:55/IgnoreNew、Deadman 06:40+PT1H/P1D/IgnoreNewがすべてenabledでActionも正規launcherへexact一致し、Pullとlegacy Runnerが不存在またはdisabledであること、NextRunTime / NumberOfMissedRuns=0、Runner Action が `-SmokeTest` / `-Status` / `-StartOnly` 等ではない本番起動 mode、Scheduled Task が watcher/bootstrap を指すこと、watcher/bootstrap 経由の実起動 canary `smoke_ok` を含む `live_runner_readiness` 証跡なしに成立しない。既存 Runner task を権限上変更できない環境では、Runner task が live runner 直叩きでも、runner 本体が本番生成前の `Assert-PreRunBootstrapInterlock` を持ち、Runner より前の 05:55 Bootstrap task が有効、NextRunTime が 05:55、NumberOfMissedRuns=0、LastTaskResult=0、Action が `-SmokeTest` / short timeout / isolated state/log を明示し、repo/live bootstrap から watcher smoke を isolated state/log で実行済みであること、かつ fresh marker 後に repo/live runner drift を検出した場合は bootstrap self-repair 後に同期済み runner へ待機付き re-exec して本番生成へ入ることを同等条件とする。pytest-static は `NEWS_GRASP_SKIP_URL_CHECK=1` と `-m "not network"` を維持し、外部 URL liveness を混ぜない。 | runner ps1、watcher ps1、bootstrap ps1、deadman launcher、state JSON、logs、gate attempts、structured repair ledger、selected artifacts、matrix scope、registry diagnostic metadata、distribution state、live runner、live watcher、live bootstrap、Task Scheduler、`build/live-runner-canary/`、`build/bootstrap-task-smoke/`。 | runner convergence / state watcher / direction ledger / matrix ownership 契約テスト、repair runtime dry-run、`tools.daily_self_heal verify-live-runner-readiness`、watcher/bootstrap/deadman contract smoke、full と RecoverOnly の両経路 dry-run。 |
...
| YouTube Podcast / publish_complete | required web/audio/deepdive の公開状態、Podcast/playlist 状態、repo/live runner SHA、repo/live watcher SHA、repo/live bootstrap SHA、Production 06:00のみ/IgnoreNew、Bootstrap 05:55/IgnoreNew、Deadman 06:40+PT1H/P1D/IgnoreNewのenabled・root TaskPath・正規Action、Pull/legacy Runner disabled-or-absent、NextRunTime / NumberOfMissedRuns=0、Runner Action 本番起動 mode、Scheduled Task watcher/bootstrap target、direct runner pre-run interlock と drift repair 後の同期済み runner re-exec、Bootstrap Action の `-SmokeTest` / short timeout / isolated state/log、watcher/bootstrap 経由の実起動 canary `smoke_ok` を確認し、非対象カテゴリ有無で完了判定を変えない。 |
...
- 実行スレッド生成前に 05:59 JST の `News-Grasp Title Materializer` が Asia/Tokyo の対象日を計算し、Codex App automation name と App DB name を exact `YY/MM/DD News-Grasp 臨時本線日次バッチ 6:00 記事作成・公開` へ反映する。正本を読む準備作業を除き、title controlの最初の実行操作は `python -m tools.news_grasp_title_materializer --verify-only --repo-root .` による read-only 検証とする。本文の `set_thread_title`、正規表現、日時 placeholder、automation 内の自己更新は使わない。materializer receipt、installed TOML、App DB、実 thread title の exact一致を確認し、stale / unavailable / failed は `title_completion=deferred` と `post_publish_issue_list` に残して非阻害とする。
