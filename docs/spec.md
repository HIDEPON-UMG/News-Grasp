...
06:00 Codex automation の通常日次公開は `$news-grasp-direct-mainline` が指定する製品内command `tools.news_grasp_direct_runtime daily` だけをLunaの入口にする direct 本線である。Codexのplugin/MCP登録やgoal再開を介さず、Windows Known Folderから解決した固定Python 3.12がruntimeを一回だけ起動し、六phaseの順序実行と状態更新を同じprocessで所有する。旧 runner、別command、NoPublish、fallback publish、runner state、readiness、durable goal、audit/report、URL 200単独、publish-status単独、commit/push単独は、この route の public completion authority ではない。legacy runner/recovery 章と矛盾する場合、06:00 direct 本線については本節を優先する。
...
### Luna Single-Command And Model Budget Contract

通常日次のLunaはskill記載の製品内command `tools.news_grasp_direct_runtime daily`を一回だけ実行する。それ以外のshell、MCP、個別operation、poll、再呼出し、run ID・SHA・manifest ID・path指定を認めない。commandはCodex automationが固定したcwdからWindows Known Folderで解決したPython 3.12を使い、direct runtimeを同一processで実行する。promotionは非terminal runが0のときだけ固定root `C:\ngstage\News-Grasp-runtime`へ行い、template、installed TOML、App DB、installed skill、snapshot、必須runtime file hashを同一source generationへ束縛してからautomationを切り替える。MCP registry、plugin marketplace、外部goal、promotion receiptの存在はDaily起動条件または完了authorityにしない。開始後のsource driftは現runを止めず次回readiness debtへ分離し、runtime改ざん、root ambiguity、許可外副作用だけを外部処理前にtyped Redとする。
...
### 場面別の音声口調（2026-09-07 ユーザー指示）

日次の「朝のニュース」朗読は、本文・導入・締めを通して自然な「です・ます調」で伝える。Web記事の常体をそのまま朗読原稿へ転記しない。DeepDiveの解説対談は会話として構成し、男性の先輩は「〜だ」「〜だね」などの口語の常体、若手は自然な敬体を使う。男性音声であることを、日次朗読まで常体にする理由にしない。

品質検査は、日次朗読とDeepDive対談の場面を区別し、話者・場面に適した口調、途中での文体混在、耳で聞いた自然さを確認する。引用文中の原文と読み上げる地の文を区別し、引用の意味や事実を文末置換で変えない。機械的な文末検査だけを自然さの合格根拠にしない。生成prompt、台本materializer、TTS前検査、保存台本の再利用判定、公開音声の確認へ同じ条件を適用し、違反時は当該台本と依存する音声・公開参照だけを修復する。
...
| Direct runtime / state / recovery | Codex automationが単一製品内command `tools.news_grasp_direct_runtime daily`を実行し、Windows Known Folderで解決した固定Python 3.12がdirect runtimeを一回起動して、V2 migration、single-flight、六Daily operation、external outbox、fresh public verifier、atomic completionを同一issue date/run intent/run IDで閉じる。旧full run、MCP、shell fallback、RecoverOnly、fallback、runner readiness、Windows Task Scheduler stateを通常日次の成功条件にしない。未知routeと旧installer起動はmutation前にfail-closedにする。 | automation template/installed/App DB/snapshot、direct runtime、runtime SQLite V2、outbox、timing ledger、consumer public observation。 | automation/direct-command parity、daily route否定fixture、single-flight/migration/atomic receiptテスト、consumer public verifier、旧installer tombstone契約テスト。 |
...
遷移receiptの発行責務は実行前の検証と実行後の結果で分離する。`tools/e2e_final_admission_bridge.py` の `validate-issued` はissued admission・policy・runner引数・実行体identityを検証してissueイベントだけを発行し、隔離worktree内のcandidate-local ownerだけが実runner process handleのcreation identity・claim・state hash・実exitを束ねた `NEWS_GRASP_E2E_RUNNER_TERMINAL_AUTHORITY_V1` を発行する。runner終了後の `record-outcome` はこのterminal authorityだけを再検証してrunner terminal receiptを発行し、callerのstate JSONやexit codeを成功証拠として受け取らない。success等の結果遷移はterminal receiptのstate hash・status・owner情報がなければappendできず、ownerはreceiptとledgerのread-only検証だけを行う。callerが作成した結果receiptや、実行前の自己申告Greenは受理しない。
...
admissionはrunner hash、引数、issue date、scheduled-equivalent intent、必須上流証跡のpath/hash/statusへ束縛する。callerが渡せる必須証拠は `efficiency_design`、`adversarial_review`、`route_manifest`、`red_suite_coverage`、`static`、`simulation`、`isolation` の7種である。`red_suite_execution` はcaller証拠として渡せず、公式admission producer自身が `tools.red_suite_execution` を一度だけ実行して8番目の証拠として挿入する。`red_suite_coverage` は固定日付の保存済みfileを再利用せず、P08 producerがcurrent sourceから一度だけ生成し、`RED_SUITE_COVERAGE_REPORT_V1`、findings空、15 Requirement、10 viewpoints、4 domain scopes、60 unique fixtures、150 pair cases、5 routes、240 traceability cells、coverage hash一致を実consumerが再検証する。`red_suite_execution` は `RED_SUITE_EXECUTION_RECEIPT_V1`、60 selectorと150 pair Red caseから実収集されたexact 211 node、collection error 0、missing outcome 0、211 passed、収集node集合hash、matrix・fixture・pair case・historical corpus・producer・pair test sourceのhash一致を必須にし、文字列Green、件数、別名関数、caller作成JSONによる自己申告を拒否する。`isolation` は `tools/e2e_isolation.py` がexact source commitから新規detached worktreeを作り、対象日artifactだけを除去し、他日artifactとsource repoを不変に保ち、runnerの既存artifact述語がfalseであることを示した当該issue date固有のreceiptでなければならない。P08はこのreceiptを明示path/hashとして一度だけ束縛し、NoPublish entry consumerがissue date、target root、source/target HEAD、artifact不在、tracked差分がreceipt記載のsanitize範囲だけであることを一度だけ検証する。isolation generatorとwrapperはGit実行前に全`GIT_*`環境を除去する。wrapperはcleanなpre-promotion candidateとissue-specific isolationのcommit・Git common-dir、source clean、隔離側tracked code不変を検証し、隔離側のP08 validatorとcandidate-local ownerだけを実行する。source HEAD/tree/clean、validator path/file hash/HEAD blobは検証時とlaunch直前に再観測する。receiptのremoved集合は正規isolation producerがsource HEADから再計算するissue-specific閉集合、policy version、set hash、`articles.jsonl`とlegacy sessionのexact transformへ束縛し、同じreceiptがissued admissionへexact path/hashで含まれる場合だけsanitized tracked差分を許可する。未束縛差分、code差分、自己申告code pathを拒否する。許可root外、既存target、commit不一致、壊れたJSONLは隔離作成前または対象artifact変更前にfail-closedとする。consumerはadmission内の自己整合だけでなく、wrapperが実際に起動する引数配列を別JSONから再読込し、順序・値・絶対pathまで完全一致しなければ `E2E_COMMAND_DRIFT` で拒否する。logical attempt Aは必須で、Aがfailure-local修正を要した場合だけ、同じissue date・scheduled-equivalent intentのlogical attempt Bを一回だけ発行・消費できる。Aが無修正で成功した場合のB、原因へ作用しない修正後のB、attempt Cは拒否する。別worktree、別receipt、別run_idで試行回数をresetしない。公式wrapperはbudget admissionを一度だけ予約し、そのreceiptをrunnerへ引き継いで同じ予約を再利用し、logical attemptごとのfinal admission消費、runner起動の順を固定する。wrapper経由でhigh-cost attemptを二重予約してはならず、`-NoPublish` のfinal E2Eは公式wrapperだけが自己予約でき、runner単独起動はfinal admission欠落として拒否する。NoPublishを必須、admission自体の再消費を禁止する。失敗原因に作用する最小修正後の同一logical attempt再開は外側policyで一回だけ許可する。存在するだけの証跡、文字列Green、caller指定ledger、product alias、並行consume、stale sourceはfail-closedにする。NoPublish Greenまでproduction runtimeへ切り替えない。

通常06:00 Codex automationはE2Eではない。通常日次は `scheduled_production`、異常終了後のproduction repair pathは `scheduled_recovery` として、final E2Eとは別identityへ固定する。通常日次はissue date単位の最大9 model callを一回だけ持ち、復旧は同じ日付identityの残予算を共有する。run ID、receipt path、worktree、session、復旧名義を変えて新しい9 callを発行してはならない。`scheduled_production` と `scheduled_recovery` はfinal E2E attemptを消費しない一方、全model callはrunnerが発行したscheduled admission receiptをbrokerへ渡し、receipt hash、issue date、task identity、scheduled attempt event、残call数をdurable ledgerで再検証してからprocessを開始する。Codex goalのE2E attemptまたはmodel call消費状態を通常日次へ流用してはならない。
...
検証はstatic inventory→schema/contract→unit/component→WAL crash/fault injection→8/14〜16 replay→production write 0のshadow→focused+broad regression→safe local candidate commit→issue-specific isolation→candidate-local ownerからfinal-only NoPublish E2E→Green後のpush/promotion/rollback rehearsalの順とする。attempt Aが内部修正なしでGreenならBは禁止し、failure-local cause hash変更時だけBを一回許可する。B後のC、patch-and-resume、publish mutationは禁止する。cutover後3回の自然scheduled cycleは別の自動集計であり、実装完了条件や後付けE2E証拠へ使わない。
