...
| Direct runtime / state / recovery | Codex automationが固定Python 3.12の単一`tools.news_grasp_daily_launcher`を起動し、V2 migration、single-flight、六Daily operation、external outbox、fresh public verifier、atomic completionを同一issue date/run intent/run IDで閉じる。旧full run、RecoverOnly、fallback、runner readiness、Windows Task Scheduler stateを通常日次の成功条件にしない。未知routeと旧installer起動はmutation前にfail-closedにする。 | automation template/installed/App DB/snapshot、direct launcher、runtime SQLite V2、outbox、timing ledger、consumer public observation。 | automation parity、daily route否定fixture、single-flight/migration/atomic receiptテスト、consumer public verifier、旧installer tombstone契約テスト。 |
