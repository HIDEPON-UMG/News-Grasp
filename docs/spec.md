...
06:00 Codex automation の通常日次公開は `$news-grasp-direct-mainline` と固定Python 3.12の `tools.news_grasp_direct_runtime daily` だけを入口にする direct 本線である。六phaseの順序実行と状態更新は同じ `tools.news_grasp_direct_runtime` processが所有する。旧 runner、NoPublish、fallback publish、runner state、readiness、durable goal、audit/report、URL 200単独、publish-status単独、commit/push単独は、この route の public completion authority ではない。legacy runner/recovery 章と矛盾する場合、06:00 direct 本線については本節を優先する。
...
runtime改ざん、state DB破損、競合live writer、許可外副作用だけは推測継続せず`failed_integrity`とする。caller supplied run ID、manifest ID、SHA、pathはactive制御authorityにせず、通常Daily入口は空入力相当の単一`tools.news_grasp_direct_runtime daily`だけとする。
...
| Direct runtime / state / recovery | Codex automationが固定Python 3.12の単一`tools.news_grasp_direct_runtime daily`を起動し、V2 migration、single-flight、六Daily operation、external outbox、fresh public verifier、atomic completionを同一issue date/run intent/run IDで閉じる。旧full run、RecoverOnly、fallback、runner readiness、Windows Task Scheduler stateを通常日次の成功条件にしない。未知routeと旧installer起動はmutation前にfail-closedにする。 | automation template/installed/App DB/snapshot、direct runtime、runtime SQLite V2、outbox、timing ledger、consumer public observation。 | automation parity、daily route否定fixture、single-flight/migration/atomic receiptテスト、consumer public verifier、旧installer tombstone契約テスト。 |
