...
DIRECT_SCHEMA_VERSION = "NEWS_GRASP_DIRECT_COMPLETION_GUARD_V1"
DIRECT_RECEIPT_SCHEMA = "NEWS_GRASP_DIRECT_MAINLINE_RECEIPT_V1"
DIRECT_STAGES = (
    "title_control",
    "issue_inventory",
    "category_collection",
...
def _direct_stage_failures(value: object) -> list[str]:
    if not isinstance(value, list):
        return ["stage_history_missing"]
    names: list[str] = []
    clocks: list[datetime] = []
    for row in value:
        if not isinstance(row, dict):
            return ["stage_history_invalid"]
        names.append(str(row.get("stage") or ""))
        parsed = _parse_clock(str(row.get("completed_at") or ""))
        if parsed is None:
            return ["stage_history_clock_invalid"]
        clocks.append(parsed)
    failures: list[str] = []
    if tuple(names) != DIRECT_STAGES:
        failures.append("stage_order_invalid")
    if any(left >= right for left, right in zip(clocks, clocks[1:])):
        failures.append("stage_clock_order_invalid")
    return failures
