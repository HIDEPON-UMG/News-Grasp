...
def _validate_live_high_cost_binding_authority(
    *,
    ...
    try:
        ...
        if len(authority_action) == 7 and authority_action[2:] == [
            "dispatch",
            "--schedule-id",
            "news-grasp-daily-v1",
            "--intent",
            ...
            binding_path = Path(str(authority.get("highCostBindingPath") or ""))
            binding_receipt_sha256 = str(authority.get("highCostBindingReceiptSha256") or "")
            if binding_path.resolve(strict=True) != expected_binding_path.resolve(strict=True):
                raise ValueError("runner binding authority mismatch")
...
def _task_launcher_action_mode(action_summary: str, *, launcher_path_text: str, mode: str) -> bool:
    ...
    return bool(
        launcher_path_text
        and launcher_path_text in action_text
        and re.search(r"(?i)(?:^|\s)dispatch(?:\s|$)", action_summary)
        and "--schedule-id news-grasp-daily-v1" in action_summary
        and "--intent reconcile" in action_summary
    )
...
def _run_live_startup_canary(
    *,
    ...
    if date > current_issue_date:
        permit_issue_date = current_issue_date
    log_file = log_dir / f"{permit_issue_date}.log"
