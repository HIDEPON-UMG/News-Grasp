---
title: "News Grasp #20260614 — Manufacturing"
date: 2026-06-14
issue: 20260614
weekday: 日曜日
category: Manufacturing
categoryId: manufacturing
accent: "#5A6B7B"
glyph: "⬢"
edition: Morning Edition
tags:
  - daily
  - newsletter
  - news-grasp
  - issue-20260614
  - cat/manufacturing
  - co/Apple
  - co/Canon
  - co/Google
  - co/JAL
  - co/Samsung
  - co/TSMC
  - co/タタ電子
  - country/インド
  - country/タイ
  - country/台湾
  - country/日本
  - country/米国
  - country/韓国
---

# ⬢ 製造 — Manufacturing

> [!summary]
> 製造カテゴリは、[[先端ノードの分散調達]]と[[拠点再編]]、そして[[規制対応]]が同時に動いた。GoogleはAI半導体の生産配分を複線化し、韓国政府はパワー半導体へ政策資金を厚くした。JALは整備能力を増強し、Canonはタイ拠点を閉じ、タタ電子には環境規制が差し込む。工場を増やす話と減らす話が同じ日に並び、供給網の強さが「どこで作るか」だけでなく「止まらず回せるか」で測られ始めている。

---

### [90] Google、次世代AI半導体をSamsung・TSMCへ分散 2nm供給網の複線化が進む

📅 2026-06-13 16:55 · 📰 finance.biggo.jp · 🔗 [元記事](https://finance.biggo.jp/news/wRj6v54BjNmBQNsNi7-d)

#cat/manufacturing #co/Google #co/Samsung #co/TSMC #country/米国 #topic/AI半導体 #event/供給網再編 #score/高

![thumb](https://img.biggo.com/Vxwo2QA9QS0dxdCCoGWviT6VY5ODrfO0H5rvTCb1HtI/fit/1720/0/sm/1/aHR0cHM6Ly9pbWcuYmdvLm9uZS9uZXdzLWltYWdlL2FpX2dlbmVyYXRlZC8yMDI2LTA2L3dSajZ2NTRCak5tQlFOc05pNy1kXzE3ODEzMzc2MzhfZGVmYXVsdC5qcGc.jpg)

- 【事実】：[[Google]]が**次世代AI半導体の生産をSamsungとTSMCへ振り分ける構え**を見せ、先端ノード調達を一社依存から外し始めた。
- 【背景】：焦点は価格よりも、__2nm世代で生産余力を複線化できるか__にある。AI向け半導体は需要の波が大きく、供給網の詰まりがそのままサービス競争力に跳ね返る。
- 【展望】：設計企業にとっては、**ファブ選択そのものが製品戦略**になる局面だ。Samsungの歩留まり改善とTSMCの逼迫度合いが次の受注配分を左右する。

---

### [87] 韓国、次世代パワー半導体R&Dに7500億ウォン 量産主導権へ政策資金を厚くする

📅 2026-06-13 13:55 · 📰 finance.biggo.jp · 🔗 [元記事](https://finance.biggo.jp/news/N9NVv54BFVfA5plgx9aC)

#cat/manufacturing #country/韓国 #topic/パワー半導体 #event/政策支援 #score/高

![thumb](https://img.biggo.com/SoHNbYalatIKmmpP4FpcApF9rpySc4LHVLBQusdj1Lg/fit/1720/0/sm/1/aHR0cHM6Ly9pbWcuYmdvLm9uZS9uZXdzLWltYWdlL2FpX2dlbmVyYXRlZC8yMDI2LTA2L045TlZ2NTRCRlZmQTVwbGd4OWFDXzE3ODEzMjY5MDZfZGVmYXVsdC5qcGc.jpg)

- 【事実】：[[韓国]]政府が**次世代パワー半導体の研究開発へ7500億ウォンを投じる方針**を打ち出し、車載・電力分野の基盤技術を国家案件として押し上げる。
- 【背景】：ここで狙うのはメモリー偏重の補完だ。__SiCや高耐圧デバイスの量産力__を厚くできれば、EVと送配電の両方で国内サプライチェーンの厚みが増す。
- 【展望】：製造業の視点では、**研究費の大小より量産移行の速度**が重要になる。装置、材料、後工程まで国内でつながるかが次の検証点だ。

---

### [83] JAL、羽田にランディングギア新整備工場 重要部位の内製整備能力を増強へ

📅 2026-06-13 17:22 · 📰 www.traicy.com · 🔗 [元記事](https://www.traicy.com/?p=373038)

#cat/manufacturing #co/JAL #country/日本 #topic/整備工場 #event/工場新設 #score/中

![thumb](https://www.traicy.com/wp-content/uploads/2026/06/JL.jpg)

- 【事実】：[[JAL]]が**羽田空港にランディングギアの新整備工場を整備**し、機体の重要保全部位を自前で回す能力を増やす。
- 【背景】：ランディングギアは整備停滞が運航制約へ直結しやすい。__処理能力を空港近接で積み増す判断__は、外部委託だけでは吸収しにくい需要を見込んだ動きだ。
- 【展望】：航空でも製造でも、**保守拠点は供給網の一部**になっている。部材調達と整備ターンアラウンドをどこまで短縮できるかが競争力を左右する。

---

### [79] Canonコラート工場が閉鎖 タイ拠点の全生産ライン終了で再編が一段進む

📅 2026-06-13 23:46 · 📰 www.thaich.net · 🔗 [元記事](https://www.thaich.net/news/20260613sz.htm)

#cat/manufacturing #co/Canon #country/タイ #topic/工場閉鎖 #event/工場閉鎖 #score/中

![thumb](https://www.thaich.net/wp-content/uploads/2026/06/Canon-AI.jpg)

- 【事実】：[[Canon]]のコラート工場が**最後の出荷を終えて全生産ラインを閉じた**。東南アジア拠点の役割分担を見直す再編が一段進んだ形だ。
- 【背景】：工場閉鎖は単なる縮小ではなく、__どの製品をどの国で作るかの再設計__を伴う。需要構成や物流費、為替を踏まえた拠点再配置の可能性が高い。
- 【展望】：量産現場では、**閉鎖の後に何を残すか**が重要になる。設備移管、人材の吸収、周辺サプライヤーへの波及が次の観測点だ。

---

### [76] タタ電子のiPhone部品工場に廃水警告 環境規制が操業リスクとして浮上

📅 2026-06-13 18:56 · 📰 jp.investing.com · 🔗 [元記事](https://jp.investing.com/news/company-news/article-1571574)

#cat/manufacturing #co/タタ電子 #co/Apple #country/インド #topic/環境規制 #event/行政警告 #score/中

![thumb](https://i-invdn-com.investing.com/redesign/images/seo/investing_300X300.png)

- 【事実】：[[タタ電子]]のiPhone部品工場が**廃水汚染を巡る警告**を受け、環境対応がそのまま操業継続リスクになる構図が見えた。
- 【背景】：インド生産の拡大局面では、__コスト優位だけでなく環境コンプライアンス__が供給網の安定度を決める。行政指摘が長引けば顧客側の調達計画にも響く。
- 【展望】：電子機器の量産では、**ESGは周辺論点ではなく生産条件**だ。現地規制対応の速さがApple向け供給の信頼性を左右する。

---

← [[2026-06-13-Manufacturing|前号 Manufacturing]] | [[2026-06-15-Manufacturing|翌号 Manufacturing]] →

*🤖 Auto-generated by News-Grasp Runner — `news-grasp-runner.bat` @ 2026-06-14 06:00 JST*
