---
title: "News Grasp #20260506 — IT & Consulting"
date: 2026-05-06
issue: 20260506
weekday: 水
category: IT & Consulting
categoryId: it
accent: "#2E6B52"
glyph: "▲"
edition: Morning Edition
tags:
  - daily
  - newsletter
  - news-grasp
  - issue-20260506
  - cat/it
  - co/NTTデータ
  - co/アクセンチュア
  - co/NEC
  - co/富士通
  - country/日本
  - country/米国
---

## ▲ 3. IT-Consulting (IT & Consulting)

> [!summary]
> NTTデータが「LITRON Builder」エージェントAI基盤でグローバル2,000件超受注、アクセンチュアはDatabricksとの専門組織設立で直販攻勢に対抗。富士通「Kozuchi Enterprise AI Factory」は7月正式展開へ。日本のSI大手がAIエージェント基盤を製品化する動きが一斉に加速している。

---

### [90] NTTデータ「LITRON Builder」でエージェントAI基盤を製品化 — グローバル2,000件超受注

📅 2026-05-06 07:00 · 📰 NTTデータ DATA INSIGHT · 🔗 [元記事](https://www.nttdata.com/jp/ja/trends/data-insight/2026/032502/)

#cat/it #co/NTTデータ #country/日本 #topic/AIエージェント #event/製品発表 #score/高

![thumb](https://raw.githubusercontent.com/HIDEPON-UMG/news-grasp-assets/main/ng-thumb-it.jpg)

- NTTデータが2026年4月から提供を開始した[[LITRON Builder]]（エージェント型AI開発基盤）のグローバル展開を加速。すでに[[2,000件以上]]のSmart AI Agentプロジェクトを受注し、シリコンバレーの新会社が北米市場開拓を主導している。
- LLMを単体で使うのではなく、複数のエージェントが役割分担して複雑業務を自律実行する「[[マルチエージェント]]アーキテクチャ」が中心。SI業界の__人月型収益モデルからの脱却__ を目指した最大の戦略転換と位置づける。
- Anthropic・OpenAIのPE直販JVが「コンサルを飛ばす」戦略であるのに対し、NTTデータは「[[日本企業の業務知識]]を強みにAIエージェント構築を内製化支援する」差別化軸を打ち出しており、顧客争奪戦の構図が鮮明になってきた。

---

### [86] アクセンチュア×Databricks：AIアプリ・エージェント導入専門組織を新設

📅 2026-05-06 07:30 · 📰 アクセンチュア · 🔗 [元記事](https://newsroom.accenture.jp/jp/news/2026/accenture-and-databricks-accelerate-enterprise-adoption-of-ai-applications-and-agents-at-scale)

#cat/it #co/アクセンチュア #country/米国 #topic/生成AI導入 #event/提携発表 #score/高

![thumb](https://newsroom.accenture.jp/jp/news/2026/media_1cc719aa877839af2a12e3a105e1c72de7f73bc08.png?width=1200&format=pjpg&optimize=medium)

- アクセンチュアとDatabricksが戦略的パートナーシップを拡大し、[[アクセンチュア-データブリックス-ビジネスグループ]]（専門組織）を設立。DatabricksをコアデータAIプラットフォームとした大規模AIエージェント導入支援を提供する。
- AI Refinery™ Distiller Agenticフレームワークと組み合わせることで、開発者が[[信頼性の高いAIエージェント]]を迅速に構築・展開できる仕組みを構築。OpenAI・Anthropic等の複数モデルを統合する「__AIエージェントの工場__」として位置づける。
- アクセンチュア株（ACN）はAnthropicのGoldman JV発表以降40%超下落しているが、今回の発表はコンサルが「モデルの直販に対抗できる技術プラットフォームを持つ」という姿勢を示す__反撃の狼煙__ とみられる。

> [!tip] 🔗 関連: 対立
> [[2026-05-05-IT-Consulting#Anthropic、コンサル業界に宣戦布告|Anthropic直販JV]] と 対立の関係。直販攻勢に対するアクセンチュアの具体的な対抗策。

---

### [83] 富士通「Kozuchi Enterprise AI Factory」— 専有型AI基盤を日本・欧州で2026年7月正式展開

📅 2026-05-06 08:00 · 📰 EnterpriseZine · 🔗 [元記事](https://enterprisezine.jp/news/detail/23590)

#cat/it #co/富士通 #country/日本 #topic/生成AI導入 #event/製品発表 #score/高

![thumb](https://ez-cdn.shoeisha.jp/static/images/article/23590/23590-top.png)

- 富士通が「[[Fujitsu Kozuchi Enterprise AI Factory]]」を日本と欧州で展開。顧客専有環境で業務特化した生成AIモデルを自律運用できる基盤として2026年2月から先行トライアル、[[7月に正式提供]]予定。
- ユーザーが独自のLLMやエージェントを開発・改善できる点が差別化要因。OSSのLLMからクローズドモデルまで選択でき、業務データで継続的にファインチューニング可能な__「専有環境でのAI自律運用」__ という新カテゴリを提唱する。
- SI業界でAIを「使う」から「作る」へのシフトが加速。富士通が独自アーキテクチャーをコード変換・業務要件の自動文書化に適用すれば、[[従来比70%の工数削減]] が可能という試算も出ており、業界の収益構造変革が不可避になる。

---

### [79] 富士通AI駆動開発がSI業態を再定義 — アクセンチュア・NTTデータ・NECとの三つ巴

📅 2026-05-06 09:00 · 📰 ITmedia オルタナティブ・ブログ · 🔗 [元記事](https://blogs.itmedia.co.jp/serial/2026/02/ainttnec.html)

#cat/it #co/富士通 #co/アクセンチュア #co/NTTデータ #country/日本 #topic/DX #score/中

![thumb](https://blogs.itmedia.co.jp/serial/assets_c/2026/02/bd17472345425c80bc9566eb7f9f7cb85e940d1a-thumb-660x562-64906.png)

- 富士通がAI駆動の開発フライホイールを本格稼働させれば、[[アクセンチュア]]・[[NTTデータ]]・[[NEC]]を駆逐するロジックが成立するとの見方が業界で広まりつつある。「AIがシステムを丸ごと開発する」富士通の戦略は人月ビジネスの根本を崩しかねない。
- NTTデータグループは2027年度に開発工程全体の40%効率化・適用案件比率50%を目標。富士通も70%工数削減を見込む。__二社の競争が業界全体の生産性基準を引き上げ__、中小SIerを市場から締め出す可能性がある。
- 外資系（アクセンチュア・Deloitte）はAIモデルベンダーとの提携でデータプラットフォームを強化し対抗。[[コンサル×SI×モデルベンダーの三者同盟]] という構図が日本のIT業界の生態系を塗り替えつつある。

---

### [75] IT大手4社、開発に生成AI適用本腰 — NTTデータG「2027年度に40%効率化」目標

📅 2026-05-06 10:00 · 📰 日経xTECH · 🔗 [元記事](https://xtech.nikkei.com/atcl/nxt/column/18/00001/11238/)

#cat/it #co/NTTデータ #co/富士通 #co/NEC #country/日本 #topic/IT投資 #score/中

![thumb](https://xtech.nikkei.com/atcl/nxt/column/18/00001/11238/topm.jpg?20220512)

- NTTデータグループ・富士通・NEC・日立の大手4社が生成AI適用を本格化。NTTデータGは「[[2027年度に40%の開発工程効率化]]」「適用案件比率50%」を目標に掲げ、現在は500件のプロジェクトに生成AIを適用中。
- GitHub CopilotなどのAIコーディングツールが先行普及した外資系コンサルに対して、日本勢は固有業務ロジックを組み込んだ「業務特化型エージェント」で差別化。__言語・法制度・業界慣行__ に精通した日本独自のデータ資産が強みとなる見通し。
- 課題はROIの可視化と人月契約の見直し。生成AI適用で「工数が減る」ことは__顧客への請求額削減__ を意味する。SI業界全体の収益構造が根本から変わる岐路に立っている。

---

*🤖 Auto-generated by News-Grasp Runner — `news-grasp-runner.bat` @ 2026-05-06 06:00 JST*
