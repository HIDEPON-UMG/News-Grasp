...
def test_unclaimed_replacement_can_be_superseded_once_without_second_attempt(
    tmp_path: Path,
) -> None:
    """claim前のauthority supersessionは同一logical E2Eの後継だけを一度許可する。"""

    original, ledger = _issue(tmp_path / "original")
    consume_admission(admission_path=original, ledger_path=ledger)
    original_value = json.loads(original.read_text(encoding="utf-8"))

    def issue_successor(name: str) -> tuple[Path, dict[str, object], Path, Path]:
        admission, _ = _issue(tmp_path / name)
        ...
        return admission, value, arguments_path, parent

    def proof_for(value: dict[str, object]) -> Path:
        return _write_json(
            tmp_path / f"proof-{value['admissionId']}.json",
            {
                ...
                "originalEvidence": {
                    "finalAdmission": {
                        ...
                        "admissionId": original_value["admissionId"],
                    }
                },
            },
        )

    def consume_successor(name: str) -> tuple[Path, dict[str, object]]:
        admission, value, arguments_path, parent = issue_successor(name)
        bridge_module.consume_admission(
            ...
            causal_replacement_proof=proof_for(value),
        )
        return admission, value

    first_replacement, first_value = consume_successor("first-replacement")
    second_replacement, second_value = consume_successor("second-replacement")
    assert first_replacement != second_replacement
    ledger_value = json.loads(ledger.read_text(encoding="utf-8"))
    row = ledger_value["attempts"][first_value["attemptKey"]]
    lineage = ledger_value["replacements"][first_value["attemptKey"]]
    assert row["admissionId"] == second_value["admissionId"]
    assert lineage["supersessionCount"] == 1
    assert lineage["supersededReplacementAdmissionId"] == first_value["admissionId"]
    assert lineage["replacementAdmissionId"] == second_value["admissionId"]
