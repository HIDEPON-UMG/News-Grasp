...
**Summary テーマの直近3日重複を避ける（2026-06-23 追加）**。執筆前に `digest/Summary/{前日}.md` / `{2日前}.md` / `{3日前}.md` が存在する場合、frontmatter の `title` / `hero_headline` / `theme` と `Today's Theme` 冒頭だけを読む。候補を最低3本作り、各候補の「同じ骨格」「主役カテゴリ」「切り口」を直近3日と比較してから採用する。大手ニュース見出しに多いタイトルパターン帳として、①主体+動作、②転換/節目、③対比/衝突、④影響/波及、⑤数字/期限、⑥現場/地域、⑦次の焦点を使い、最低3系統を候補に混ぜる。最終採用は単一の主役ニュースの見出しとし、主体・出来事・動作または結果を明示する。`広がる入口、狭める境界` のような抽象標語は禁止する。複数の独立ニュースを `と` や `・` で接合しない。総括本文は横断的でよいが、見出しは最重要ニュース1件を要約する。

```jsonc
{
  "title": "日米が円買い協調介入、ドル円は一時155円台前半へ", // 単一の主役ニュース（12〜42字）
  "subtitle": "...",                     // サブタイトル（30〜50 字）

  // ↓ title と完全一致する単一の主役ニュース見出し。
  // 複数の独立ニュースを「と」「・」で接合しない。抽象標語にしない。
...
- **`hero_headline` を必ず frontmatter に出力**。単一の主役ニュースについて主体・出来事・動作または
  結果が分かる12〜42字の見出しとし、title の em dash 後と完全一致させる。複数の独立ニュースを
  `と` や `・` で接合しない。`hero_left` / `hero_right` は新規出力しない。
