...
- **Summary テーマの直近3日重複を避ける（2026-06-23 追加）**。執筆前に `digest/Summary/{前日}.md` / `{2日前}.md` / `{3日前}.md` が存在する場合、frontmatter の `title` / `hero_headline` / `theme` と `Today's Theme` 冒頭だけを読む。全文 Read は不要。候補を最低3本作り、各候補について「骨格」「主役カテゴリ」「切り口」を直近3日と比較してから 1 本だけ採用する。
  ...
  - **最終採用は単一の主役ニュースの見出しにする**。タイトルだけで主体・出来事・動作または結果が分かるよう、当日の固有名詞、政策、製品、数字、地域のいずれかを最低1つ出す。`広がる入口、狭める境界` のような評論標語は禁止する。複数の独立ニュースを `と` や `・` で接合したカテゴリ横断標語も禁止する。総括本文は横断的でよいが、見出しは最重要ニュース1件を要約する。
...
- **`hero_headline` を必ず frontmatter に出力**。単一の主役ニュースについて主体・出来事・動作または結果が分かる12〜42字の見出しとし、`title` の `—` 後と完全一致させる。例: `日米が円買い協調介入、ドル円は一時155円台前半へ`。`hero_left` / `hero_right` は新規出力しない。
...
日次朗読の導入・本文・締めは自然な「です・ます調」で統一する。男性話者でも常体にしない。Web記事やSummaryの常体をそのまま転記せず、事実・数字・引用の意味を保った朗読文にする。DeepDive解説対談の先輩の「〜だ」「〜だね」という口調はこの日次朗読へ適用しない。文体混在と耳で聞いた不自然さを確認してから提出する。
...
朗読本文を書く前に、同じファイルの本文冒頭へ HTML コメント形式の **論点設計メモ** を必ず置く。このブロックは品質 gate が読むための設計情報であり、後続の TTS 正規化では読み上げ本文から除外される。形式は必ず `<!-- tts-outline` で始め、`-->` で閉じる。中には最低限、`中心論点`、`背景`、`なぜ今`、`因果関係`、`カテゴリ論点`、`リスク・未確定`、`次の観測点` をこの表記で含める。`カテゴリ論点` には scheduled_categories の各カテゴリ ID（例: `fx` / `ai` / `it` / `mobility` / `manufacturing` / `economy` / `game`）を1行ずつ置き、そのカテゴリで「どの事実から、どの制約・影響・次の観測点へ踏み込むか」を短く書く。これは公開表示用の前置きではなく、原稿生成前の設計図である。

`tts-outline` の中心論点を決めるまでは本文を書き始めない。中心論点は、単なる共通語やカテゴリ横断のラベルではなく、今日の複数ニュースが同じ構造を別の場所で示していることを 1 文で説明する。背景は「なぜ今日この並びが起きたか」、因果関係は「発表・数字・制度・供給制約が、現場や利用者にどう波及するか」、リスク・未確定は「まだ断定できない条件」、次の観測点は「明日以降どのシグナルを見れば論点が進んだと言えるか」を書く。
...
字数不足はテーマ深掘り不足として扱う。原稿が 2,500 字に届かない場合、同型の補足文やカテゴリ名だけを差し替えた文で埋めない。必ずテーマ設計に戻り、各カテゴリで「なぜ今このニュースなのか」「どの制約や前提が変わったのか」「次に観測すべきシグナルは何か」を具体化してから書き直す。1 カテゴリにつき最低 1 つは、数字・期限・主体・制度・供給制約・利用場面のいずれかを使って、事実から判断軸へ踏み込む。横断テーマは、単なる共通語の列挙ではなく、複数カテゴリの出来事が同じ構造を別の場所で示していることを説明する。字数合わせの補足文で埋めない。
