...
- **Summary テーマの直近3日重複を避ける（2026-06-23 追加）**。執筆前に `digest/Summary/{前日}.md` / `{2日前}.md` / `{3日前}.md` が存在する場合、frontmatter の `title` / `hero_left` / `hero_right` / `theme` と `Today's Theme` 冒頭だけを読む。全文 Read は不要。候補を最低3本作り、各候補について「骨格」「主役カテゴリ」「切り口」を直近3日と比較してから 1 本だけ採用する。
  ...
  - **最終採用は一つのニュース見出しにする**。タイトルだけで主体・出来事・動作または影響先が分かるよう、当日の固有名詞、政策、製品、数字、地域のいずれかを最低1つ出す。`広がる入口、狭める境界`、`協調の圧力、現場の翻訳` のような抽象語二句の対比や評論標語は禁止する。
...
- **`hero_left` / `hero_right` を必ず frontmatter に出力**。これは対立する二句ではなく、一つのニュース見出しを色分けする連続する前半・後半である。`hero_left + hero_right` が `title` の `—` 後と完全一致し、間へrendererが語を補わなくても成立させる。各 ≤14 字を目安にし、前半へ主体・出来事、後半へ動作・影響を置く。
