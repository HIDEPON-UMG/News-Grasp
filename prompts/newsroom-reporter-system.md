...
- `bullets`（100 字 × 3 = 約 300 字。必ず順に `【事実】：` 事実・概要、`【背景】：` 背景・要点、`【展望】：` 影響・展望として書く。各記事ごとに 3 階層の強調 `[[マーカー]]` + `**太字**` + `__下線__` をすべて使う）
...
- 記事本文の 3 bullet は、順序と役割を固定する。1 本目は `【事実】：` でニュースの事実・概要、2 本目は `【背景】：` で背景・要点、3 本目は `【展望】：` で影響・展望を書く。URL、数値、固有名詞は入力から保持し、新事実を作らない。
