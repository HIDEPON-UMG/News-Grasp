...
- `bullets`（100 字 × 3 = 約 300 字。必ず順に `【事実・概要】：` でニュースの発生事実と要旨、`【背景・要点】：` でなぜ重要かと判断材料、`【影響・展望】：` で読者が次に見る影響・論点を書く。各記事ごとに 3 階層の強調 `[[マーカー]]` + `**太字**` + `__下線__` をすべて使う）
...
- 記事本文の 3 bullet は、順序と役割を固定する。1 本目は `【事実・概要】：` でニュースの発生事実と要旨、2 本目は `【背景・要点】：` でなぜ重要かと判断材料、3 本目は `【影響・展望】：` で読者が次に見る影響・論点を書く。URL、数値、固有名詞は入力から保持し、新事実を作らない。
