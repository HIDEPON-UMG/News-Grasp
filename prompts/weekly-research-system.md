...
#### `## 深掘り` — データ駆動の論点（**数値変遷チャート最低 1 ＋ データ表 1・出典必須**）

このテーマの核心を**定量データ**で示し、そこから**論点**を立てる。**「数値の変遷（時系列の動き）」と「数値の一覧（表）」を必ず両方**出す。

- ` ```chart ` ブロックを**最低 1 つ**置く（`source` 必須。出典の無い図は禁止）。**うち最低 1 つは「数値の変遷」を示す時系列チャート**（`line`、または時間カテゴリの `bar`/`stacked_bar`。**3 点以上の実データ**で、projection（目標値）だけで埋めない）:
  ```chart
  {
    "type": "stacked_bar",
    "title": "図のタイトル",
...
| チャート | `chart` | `type` / `title` / `series` / `source`（+ `unit` / `categories` / `annotations`） | 深掘り | **最低 1（うち 1 つは時系列変遷）** |
| データ表 | `table` | `columns` / `rows` / `source` | 深掘り | **1（必須）** |
| 意思決定 | `decision` | `issue` / `options` / `deadline` / `decider` | 注目点 | 1 |
