...
モデル方針は `tools/model_policy.py` を正本とします。2026-07-10 の各5回実測では、記者は `gpt-5.4` を維持し、必要時の文体調整は `gpt-5.6-luna`、編集長本体は `gpt-5.6-terra`、DeepDive は `gpt-5.6-sol` を採用します。文体調整は全記事一律ではなく、自然さ・News-Grasp らしさ・validator fail のいずれかで弱い記事だけに限定してください。
