...
function Recover-NewsGraspInterruptedInstall {
    ...
    foreach ($journalFile in @(Get-ChildItem -LiteralPath $BackupRoot -Filter 'install-manifest.json' -File -Recurse | Sort-Object LastWriteTime)) {
        try {
...
Recover-NewsGraspInterruptedInstall -BackupRoot $backupRoot
