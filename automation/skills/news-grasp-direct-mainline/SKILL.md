...
実行操作は次のcommandを一回だけ起動することに限る。個別phase、管理command、raw script、旧runner、NoPublish、historical、Playwright、full test、手動repair、pollingを開始しない。

```text
C:\Users\hidek\AppData\Local\Programs\Python\Python312\python.exe -m tools.news_grasp_direct_runtime daily
```

commandが返したmachine receiptを日本語で報告する。原因調査、別経路への切替え、run ID・SHA・manifest IDの指定、同じcommandの再実行をLunaが判断しない。`public incompleteかつexact successorがある状態で終了しない`責任はruntime brokerが持ち、Lunaは低位operationを列挙・実行しない。
...
- 45分で公開critical pathへ限定し、75分以降は新規polishを開始しない。SLOは90分だが、登録済みsuccessorがあれば停止しない。
